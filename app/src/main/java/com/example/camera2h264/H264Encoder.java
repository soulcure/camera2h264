...
//            mediaCodec = MediaCodec.createEncoderByType("video/avc");
            mediaCodec = MediaCodec.createByCodecName("c2.rk.avc.encoder");
            //height和width一般都是照相机的height和width。
            //TODO 因为获取到的视频帧数据是逆时针旋转了90度的，所以这里宽高需要对调
            MediaFormat mediaFormat = MediaFormat.createVideoFormat("video/avc", width, height);

            mediaFormat.setInteger(MediaFormat.KEY_BITRATE_MODE, 3);
            //描述平均位速率（以位/秒为单位）的键。 关联的值是一个整数
//            mediaFormat.setInteger(MediaFormat.KEY_BIT_RATE, width * height * 5);
            mediaFormat.setInteger(MediaFormat.KEY_BIT_RATE, 2000);
...
//            mediaFormat.setInteger(MediaFormat.KEY_FRAME_RATE, frameRate);
    ...
    public void encoderH264(byte[] data) {
        ...
        try {
            ...
            //Log.e(TAG, "encoderH264: encode error!!!");
            t.printStackTrace();
        }
    }
